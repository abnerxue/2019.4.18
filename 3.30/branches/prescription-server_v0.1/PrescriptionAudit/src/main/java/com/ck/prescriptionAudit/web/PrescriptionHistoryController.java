...
public class PrescriptionHistoryController {
	...
	//审核报表
	@GetMapping(value = "/getreportsummary")
	 public Map<String, Object> getReportSummary (String beginDateScope) throws Exception {
			Map<String, Object> map = new LinkedHashMap<>();
			Map<String, Object> map1 = new LinkedHashMap<>();
			
		   List<reportSummary> rep = prescriptionHistoryService.getReportSummary(beginDateScope);

		   int preTotalNumber = prescriptionHistoryService.getReportSummaryCount(beginDateScope);
		   
		   for(int i = 0;i<rep.size();i++) {
			   rep.get(i).setId(i+1);
			   double percent = (double)rep.get(i).getNumber()/preTotalNumber * 100;
			   DecimalFormat r=new DecimalFormat();  
			   r.applyPattern("#0.00");
			   rep.get(i).setPercentage(r.format(percent) + "%");
		   }
		   map1.put("totalNumber", preTotalNumber);
		   map1.put("result", rep);
		   
		   if (map1.size()==0 || map1.isEmpty()){
			   map.put("code",0);
			   map.put("msg","该时间段无处方信息");
			   map.put("result", map1);
		   }else if(map1.size()>0){
		   map.put("code",0);
		   map.put("msg","success");
		   map.put("result", map1);		   
			   ...
			   map.put("code",10000);
			   map.put("msg","获取处方信息失败"); 
		   }    
		   return map;
 }
	//审核报表详细信息
	@GetMapping(value = "/getreportsummarydetail")
	 public Map<String, Object> getReportSummaryDetail (@RequestParam(defaultValue = "1") Integer pageNo,
		        @RequestParam(defaultValue = "10") Integer pageSize,String beginDateScope,
		        Integer  totalCount,Integer totalPage) throws Exception {
		Map<String, Object> map = new LinkedHashMap<>();
		
	   List<PrescriptionHistoryReportDetail> rep = prescriptionHistoryService.getReportSummaryDetail(pageNo,pageSize,beginDateScope);
	   List<PrescriptionHistoryReportDetail> repPage = new ArrayList<PrescriptionHistoryReportDetail>();
	   for(int i = 0;i<rep.size();i++) {
		   List<ProblemCode> pro = prescriptionHistoryService.getProblemById(rep.get(i).getPrescriptionNumber());

		   StringBuilder problemName = new StringBuilder();
		   List<String> auditResult = new ArrayList<String>();
		   for(int j = 0;j<pro.size();j++) {
			   problemName.append((j+1) + "、" + pro.get(j).getProblemName() + "  ");
			   auditResult.add(pro.get(j).getAuditResult());
		   }
		   if(auditResult.contains("打回")) {
			   rep.get(i).setAuditResult("打回");
		   }else if(!auditResult.contains("打回") && auditResult.contains("提醒")) {
			   rep.get(i).setAuditResult("提醒");
		   }
		   rep.get(i).setProblemName(problemName.toString());		   
	   }
	   
		if(rep.size()>pageSize) {
		for(int j =(pageNo-1)*pageSize;j<pageNo*pageSize;j++) {
			repPage.add(rep.get(j));
		}
		}else {
	   ...
	   totalCount = prescriptionHistoryService.getReportSummaryCount(beginDateScope);
	   totalPage = totalCount%pageSize==0?totalCount/pageSize:totalCount/pageSize + 1;
	   Page p = new Page(pageNo,pageSize,totalPage,totalCount);
	   if (repPage.size()==0 || repPage.isEmpty()){
		   map.put("code",0);
		   map.put("msg","该时间段无处方信息");
		   map.put("result", repPage);
	   }else if(rep.size()>0){
	   map.put("code",0);
	   map.put("msg","success");
	   map.put("page",p);
