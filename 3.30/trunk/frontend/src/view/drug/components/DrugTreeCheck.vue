...
export default {
  ...
  async created() {
    ...
    await this.formatList(this.list, 't0');
  ...
  methods: {
    ...
    toFold() {
      ...
      for (let i in this.$refs.tree.store._getAllNodes()) {
        if (this.$refs.tree.store._getAllNodes()[i].data.id === 't0') {
    ...
    formatMedcine() {
      this.allmedcine.forEach((medcine) => {
        this.allcategory.forEach((category) => {
          if ('t' + medcine.subcategory == category.id) {
            medcine.subcategory = 't' + medcine.subcategory;
