<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="level" label="审方等级" width="100" align="center"></el-table-column>
      <el-table-column prop="enable" label="操作" align="center" width="80">
        <template slot-scope="scope">
          <i @click="showEdit(scope.row)" class="el-icon-edit pointer"></i>
          <span class="d-inline-block" style="width: 8px;"></span>
          <i @click="handleDelete(scope.row.ruleName)" class="el-icon-delete text-red pointer"></i>
        </template>
      </el-table-column>
    </el-table>
    <CheckModal ref="modal" :title="'编辑规则'" :detail="detail" @click="handleEdit"/>
  </div>
</template>
