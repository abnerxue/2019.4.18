...
public class MedcineRestrictionController {
	...
	@GetMapping(value = "/selectapplicablebydiagnosisid")
	public Map<String, Object> selectApplicableByDiagnosisId(Integer diagnosisId) {

		Map<String, Object> map = new LinkedHashMap<>();
		List<ApplicableMedicine> applicablemedicine = medcineRestrictionService.selectApplicableByDiagnosisId(diagnosisId);
		if (applicablemedicine.size() == 0) {
			map.put("code", 0);
			map.put("msg", "该诊断无适用药品信息");
			map.put("result", applicablemedicine);
		} else if (applicablemedicine.size() > 0) {
			map.put("code", 0);
			map.put("msg", "success");
			map.put("result", applicablemedicine);
		} else {
			map.put("code", 10000);
			map.put("msg", "获取该诊断无适用药品失败");
		}
		return map;
	}
	...
	@RequestMapping(value = "/deleteapplicablebyid", method = RequestMethod.POST)
	public RespBean deleteApplicableById(Integer id) {
		if (medcineRestrictionService.deleteApplicableById(id) == 1) {
			return RespBean.ok("删除成功!");
		}
		return RespBean.error("删除失败!");
	}
	...
	@RequestMapping(value = "/insertdiagnsisapplicable", method = RequestMethod.POST)
	public RespBean insertDiagnsisApplicable(Integer diagnosisId, Integer medcineId) {
		
		if (medcineRestrictionService.repeatVerification(diagnosisId, medcineId) > 0) {
			return RespBean.error("添加失败,该诊断适用药物中已存在该药物!");
		} else {
			if (medcineRestrictionService.insertDiagnsisApplicable(diagnosisId, medcineId) == 1) {
				return RespBean.ok("添加成功!");
			} else {
				return RespBean.error("添加失败!");
			}
