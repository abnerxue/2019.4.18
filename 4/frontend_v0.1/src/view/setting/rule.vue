...
<script>
import { AllRangeConfig, UpdateRangeConfig, SelectSetupRule } from '@/api/setting'
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      if (!re.test(this.data.value0)) {
        this.$message.error('药物价格最高必须为正整数');
        return false;
      } else if (!re.test(this.data.value1)) {
        this.$message.error('处方内西药或中成药最大种类数必须为正整数');
        return false;
      }
      this.loading = true
    ...
    // 获取全部系统设置
    async getAllConfig() {
      const res = await SelectSetupRule()
      if (res.code === 0) {
