...
const {
  ...
  selectapplicablebydiagnosisid, // GET 根据诊断表的id获取适用药品列表
  deleteapplicablebyid, // POST  通过id删除药物适用表
  insertdiagnsisapplicable, // POST 添加药品适用
  // selecttreatmentbydiagnosisid, // GET 根据诊断表id查询疗程信息
  // updattreatmentbyid, // POST 保存疗程
  // 药品属性和通用规则
  ...
  // 适应症限用
  getallindicationbydiagnosisid, // 获取适用症列表
  updatetpathologyphysiologyindication, // 修改病生理
  updatetgroupindication, // 修改人群适应症
  updateageindication, // 修改年龄适应症
  updatetgestationalweekindication, // 修改孕周适应症
  selectallcompatibility, // GET 获取所有药品、分类、标签列表
  selectreaction // 作用机制与结果智能联想输入
} = apiUrls.dataMana
...
// 规则管理 - 配伍禁忌 - 作用机制与结果智能联想输入
export async function GetReaction(args) {
  const res = await fetch(selectreaction, args, 'GET')
  return res
}
// 规则管理 - 配伍禁忌 - 获取所有药品、分类、标签列表
export async function GetAllCompatibility(args) {
  const res = await fetch(selectallcompatibility, args, 'GET')
  return res
}
// 规则管理 - 配伍禁忌
...
// 适应症限用 获取适用症列表
export async function GetAllIndicationByDiagnosisId(args) {
  const res = await fetch(getallindicationbydiagnosisid, args, 'GET')
  return res
}
// 适应症限用 修改病生理
export async function UpdatePhysiology(args) {
  const res = await fetcher(updatetpathologyphysiologyindication, args)
  return res
}
// 适应症限用 修改人群适应症
export async function UpdateGroup(args) {
  const res = await fetcher(updatetgroupindication, args)
  return res
}
// 适应症限用 修改年龄适应症
export async function UpdateAge(args) {
  const res = await fetcher(updateageindication, args)
  return res
}
// 适应症限用 修改孕周适应症
export async function UpdateWeek(args) {
  const res = await fetcher(updatetgestationalweekindication, args)
  return res
}

// 适用药及疗程维护 根据诊断表的id获取适用药品列表
export async function GetApplicable(args) {
  const res = await fetch(selectapplicablebydiagnosisid, args, 'GET')
  return res
}
// 适用药及疗程维护 通过id删除药物适用表
export async function DeleteApplicable(args) {
  const res = await fetch(deleteapplicablebyid, args)
  return res
}
// 适用药及疗程维护 添加药品适用
export async function AddApplicable(args) {
  const res = await fetch(insertdiagnsisapplicable, args)
  return res
}
