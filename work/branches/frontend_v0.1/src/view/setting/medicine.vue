...
<script>
import { AllRangeConfig, UpdateRangeConfig } from '@/api/setting'
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      if (!re.test(this.data.value0)) {
        this.$message.error('每次极量必须为正整数');
        return false;
      } else if (!re.test(this.data.value1)) {
        this.$message.error('每日极量必须为正整数');
        return false;
      }
      this.loading = true
    ...
    // 获取全部系统设置
    async getAllConfig() {
      const res = await AllRangeConfig()
      if (res.code === 0) {
