  ...
  <div class="drug-tag">
    ...
    >
        ...
        >
          ...
          <div>
            <el-row
              ...
              :key="index"
            >
              <span class="label-name" @click="handleClick(item)">{{ item.name }}</span>
              <span class="label-del" @click="delLabel(item.id)">删 除</span>
              <span class="label-edit" @click="showLabelModal('edit', item)">编 辑</span>
            </el-row>
          </div>
        </el-card>
        </div>
      ...
      >
        ...
        >
          ...
          <el-row style="text-align: center; padding-top: 15px; font-size: 14px;" v-if="medicinesMsg != 'success'">
    ...
    >
      ...
      <el-input
        size="small"
        placeholder="请输入标签名称"
        v-model="labelModal.data.name">
      </el-input>
      <span slot="footer" class="dialog-footer">
        <el-button @click="hideLabelModal" size="small">取 消</el-button>
        <el-button type="primary" @click="saveLabelModal('add')" size="small" v-if="!labelModal.data.id">保 存</el-button>
        <el-button type="primary" @click="saveLabelModal('edit')" size="small" v-if="labelModal.data.id">保 存</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    // 获取标签列表
    async getLabelList(val) {
      const res = await GetLabelList(val);
      if (res.code === 0) {
