...
<template>
  <div style="background: #282c35;color: #fff;height:calc(100vh - 100px);position: absolute;top: 80px;min-width: 625px;">
    <div style="background: #18191D;max-height: calc(100vh- 120px);margin-left: 20px; margin-top: 20px;width:calc(100vw - 320px);height:97%;">
      ...
      <div>
        <div style="display: flex;min-width: 600px;flex-direction: row;align-items: center;justify-content: space-around;position: relative;margin-left: -30px;margin-top: 30px;margin-bottom: 15px;">
...
export default {
  ...
  mounted() {
    ...
    if (window.innerHeight < 900) {
      $("#vehicleMonitorTable").css(
        "height",
        (window.innerHeight * 600) / 980 + "px"
      );
  ...
  methods: {
    ...
    getTableData(pageIndex) {
      ...
      this.$http({
        method: "post",
        url: "/gm-data/api/interface/carmonitor",
        headers: {
      ...
      timeOut = setTimeout(this.getTableData, 600000);
    }
  }
};
</script>
